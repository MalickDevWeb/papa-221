...
async function runTests() {
  ...
  const scanResult = await assertEndpoint('Scan badge étudiant (Paiement en retard)', 'POST', '/vigil/scan', { badgeId: '221-M382' });
  console.log('   Resultat du Scan:', JSON.stringify(scanResult));

  // Test 2-minute delay rule simulating 1 hour (using student matricule "221-M383" with scolarité à jour)
  console.log('\n--- Test de la règle du délai de 2 minutes (simulant 1h) entre scans ---');
  const firstScan: any = await assertEndpoint('Premier Scan', 'POST', '/vigil/scan', { badgeId: '221-M383' });
  console.log('   Premier Scan:', JSON.stringify(firstScan));

  // If the first scan was a departure, we need to do another scan to put the student in "arrivée" status,
  // so we can then test that an immediate departure scan is blocked.
  if (firstScan.message && firstScan.message.includes('(départ)')) {
    const intermediateScan: any = await assertEndpoint('Scan Intermédiaire (Arrivée)', 'POST', '/vigil/scan', { badgeId: '221-M383' });
    console.log('   Scan Intermédiaire:', JSON.stringify(intermediateScan));
  }

  const secondScan: any = await assertEndpoint('Deuxième Scan Immédiat (Refusé)', 'POST', '/vigil/scan', { badgeId: '221-M383' });
  console.log('   Deuxième Scan:', JSON.stringify(secondScan));
