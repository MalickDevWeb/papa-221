...
export function StudentQRZoomOverlay({ studentName, matricule, onClose }: Props) {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(mat)}-${encodeURIComponent(name.toUpperCase().replace(/\s+/g, '-'))}&color=1b1c1e&margin=4`;
