...
export function PlanningSidebar({ rooms }: Props) {
  ...
  const handleDragStart = (e: React.DragEvent, type: 'teacher' | 'room', name: string) => {
    e.dataTransfer.setData('application/json', JSON.stringify({ type, name }));
  };

  return (
    <div className="w-64 bg-white border border-[#E2DCDA] rounded-2xl p-4 flex flex-col gap-3 shadow-sm shrink-0">
      <div className="flex border-b border-neutral-100 pb-1">
        <button
          onClick={() => setActiveTab('teachers')}
          className={`flex-1 pb-2 text-xs font-black uppercase text-center border-b-2 transition-all cursor-pointer ${
            activeTab === 'teachers' ? 'border-[#B3181C] text-[#B3181C]' : 'border-transparent text-neutral-400'
          }`}
        >
          Enseignants
        </button>
        <button
          ...
          className={`flex-1 pb-2 text-xs font-black uppercase text-center border-b-2 transition-all cursor-pointer ${
            activeTab === 'rooms' ? 'border-[#B3181C] text-[#B3181C]' : 'border-transparent text-neutral-400'
          }`}
        >
          Salles
        </button>
          ...
          rooms.map((r) => (
            ...
            >
              ...
              <div>
                ...
                <span className="text-[9px] text-emerald-600 font-bold">Capacité: {r.capacity}</span>
              </div>
            </div>
          ))}
      </div>
