...
export function StepPersonal({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) setNomFichierCni(e.target.files[0].name);
  };

  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault(); setIsDragging(false);
    if (e.dataTransfer.files && e.dataTransfer.files[0]) setNomFichierCni(e.dataTransfer.files[0].name);
  };

  const canContinue = nomComplet.trim() && email.trim() && telephone.trim() && numeroCni.trim() && (!cniRequis || nomFichierCni !== 'Non fourni');
      ...
      {cniRequis && (
        <div className="space-y-1" id="cni-upload-container">
          ...
          <div onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop} className={`border-2 border-dashed rounded-xl p-3 text-center transition-all cursor-pointer relative ${isDragging ? 'border-[#B3181C] bg-[#FFF5F5]' : nomFichierCni !== 'Non fourni' ? 'border-[#1E5E3A] bg-[#EAF7EE]/40' : 'border-[#E2DCDA] hover:bg-[#FAF8F6]'}`}>
            ...
            <span translate="no" className={`material-symbols-outlined text-2xl ${nomFichierCni !== 'Non fourni' ? 'text-[#1E5E3A]' : 'text-[#8E7977]'}`}>{nomFichierCni !== 'Non fourni' ? 'verified' : 'contact_emergency'}</span>
            <p className="text-[11px] font-bold text-[#3E2927] mt-1">{nomFichierCni !== 'Non fourni' ? nomFichierCni : 'Glissez-déposez votre CNI ou cliquez'}</p>
