...
import { ShieldAlert, Users, Calendar, Video } from 'lucide-react';
...
export function AdminGlobalMeetings({ meetings, onCreate }: Props) {
  ...
  const [details, setDetails] = useState('Tous les départements');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onCreate(title, 'Direction Académique', scope, details, type, meetLink);
    setTitle('');
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-neutral-gray-200 p-6 shadow-sm space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-bold text-base text-gray-900">Réunions Institutionnelles & Événements (Administration)</h3>
          <p className="text-xs text-neutral-500">Créez des webinaires, séminaires, soutenances ou réunions générales</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              >
                ...
                <option value="soutenance">Soutenance de thèse</option>
                <option value="webinaire">Webinaire de recherche</option>
              </select>
            </div>
          </div>
        ...
        <div className="space-y-3 overflow-y-auto max-h-[220px] no-scrollbar">
          <span className="text-[10px] font-bold text-neutral-600 block">Événements publiés de l&apos;établissement</span>
          {meetings.map((m) => (
            <div key={m.id} className="p-3 rounded-xl border border-neutral-200 flex justify-between items-center bg-white shadow-3xs">
              <div>
                ...
                <span className="text-[10px] text-neutral-500 font-semibold">{m.targetScope} - {m.targetDetails}</span>
              </div>
              <a href={m.meetLink} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-[#4285F4] text-white font-bold text-[10px] rounded-xl flex items-center gap-1 hover:bg-[#357ae8]">
